                             ...
                             const char *encoding, FILE *output)
{
	...
	fprintf(stderr, "V: %ld", ftell(output));
...
void Usage(const char *argv0)
{
    ...
    printf("Usage: %s [-r fps] [-c channels] [-n input] [-o offset] [-1]\n", argv0);
...
int main(int argc, char *argv[])
{
    ...
    fps_set = 0;
    strcpy(input_names, "frame%d.jpg");    
    ...
    for ( index=1; argv[index]; ++index ) {
		...
		if ( (strcmp(argv[index], "-o") == 0) && argv[index+1] )
        {
            index++;
			frame_offset = atoi(argv[index]);
        }
    ...
    if ( video_nframes > 0 ) {
        sprintf(jpegfile, input_names, frame_offset);
    ...
    for ( index=frame_offset; index <= video_nframes; ++index ) {
