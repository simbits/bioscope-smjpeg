...
int SMJPEG_load(SMJPEG *movie, const char *file)
{
    ...
    do {
        ...
        if ( MAGIC_EQUALS(buffer, VIDEO_HEADER_MAGIC) ) {
			...
			movie->video.frame_offsets = (long*)malloc(movie->video.frames * sizeof(long));
			if (movie->video.frame_offsets == NULL) {
				SMJPEG_status(movie, -1, "Out of memory allocating frame offsets\n");
				goto error_return;
			}
            movie->video.frame = 0;
    ...
    do {
        ...
        if ( fread(magic, 4, 1, movie->src) ) {
            if ( MAGIC_EQUALS(magic, DATA_END_MAGIC) ) {
				...
				movie->video.frame_offsets[movie->video.frame] = pos;
                READ32(movie->current, movie->src);
                READ32(length, movie->src);
				//printf("pos: %ld -> 0x%.4x\n", movie->video.frame, pos);
				//printf("current: %ld, length: %ld\n", movie->current, length);
            }
        }
    } while ( !END_OF_STREAM(movie, magic) && (movie->video.frame < movie->video.frames) );
...
int SMJPEG_inc_frame(SMJPEG *movie)
{
	int status;
	long pos;

	if (++movie->video.frame >= movie->video.frames) {
		movie->video.frame = 0;
	}
...
int SMJPEG_dec_frame(SMJPEG *movie)
{
	int status;
	long pos;

	if (movie->video.frame == 0 || movie->video.frame >= movie->video.frames) {
		movie->video.frame = movie->video.frames;
	}
